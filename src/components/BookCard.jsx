...
const BookCard = ({ book, onEdit, onDelete, isLocal = false }) => {
  const { getBookData, cart } = useContext(CartContext);
  const { toggleFavorite, isFavorite } = useContext(FavoritesContext);
  const navigate = useNavigate();

  const id = isLocal ? book.id : book.key;
  let coverUrl = isLocal 
  ...
  // Parche de seguridad: Evitar que imágenes Base64 gigantes del pasado congelen la UI
  if (coverUrl && coverUrl.length > 300000 && coverUrl.startsWith('data:image')) {
    coverUrl = 'https://via.placeholder.com/300x400?text=Imagen+Demasiado+Pesada';
  }
  ...
  useEffect(() => {
    ...
    setCondition(isLocal ? book.condition : data.condition);
    setEbookData(data.ebook);
  }, [book, id, isLocal, getBookData]);
  ...
  const getStockBadge = () => {
    ...
    if (remainingStock === 0) return <span className="stock-badge bg-danger text-white">Agotado</span>;
    if (remainingStock <= 2) return <span className="stock-badge bg-warning text-dark">Pocas unidades</span>;
  ...
  const getBadges = () => {
    return (
      <div className="d-flex flex-wrap gap-1">
        {ebookData.isEbook ? (
          <span className="badge bg-dark mb-2"><i className="bi bi-phone me-1"></i>E-Book ({ebookData.format})</span>
        ) : (
          <>
            <span className={`badge ${condition === 'nuevo' ? 'bg-primary' : 'bg-secondary'} me-2 mb-2 text-capitalize`}>{condition}</span>
            {remainingStock > 0 && (
              <span className="badge bg-secondary mb-2">Stock: {remainingStock}</span>
            )}
          </>
        )}
      </div>
    ...
    <div className="card book-card h-100 border-0 shadow-sm">
      ...
      <div className="card-body d-flex flex-column">
        ...
        <div className="mt-auto">
          <p className="fw-bold text-magenta fs-5 mb-2">${(price || 0).toLocaleString()}</p>
          
          {isLocal && onEdit && onDelete ? (
            <div className="d-flex justify-content-between gap-2">
              <button className="btn btn-outline-magenta btn-sm flex-grow-1" onClick={(e) => { e.stopPropagation(); onEdit(book); }}>
                <i className="bi bi-pencil"></i>
              </button>
              <button className="btn btn-outline-danger btn-sm flex-grow-1" onClick={(e) => { e.stopPropagation(); onDelete(id); }}>
                <i className="bi bi-trash"></i>
              </button>
              <Link to={`/book/${encodeURIComponent(id)}`} state={{ book: { ...book, isLocal, coverUrl, title, author, condition, ebookData } }} className="btn btn-magenta btn-sm flex-grow-1">
                Ver más
              </Link>
            </div>
          ) : (
